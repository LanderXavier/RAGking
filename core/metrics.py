...
def compute_metrics(
    ...
    # Cost per query and Economic Efficiency (Correctness / USD)
    ...
    # Token Efficiency = (Tin + Tout) / max(Correctness, 1)
    ...
    df["economic_eff_raw"] = df.apply(lambda r: float(r.get("Correctness") or 0.0) / (r["_cost_safe"] if pd.notna(r["_cost_safe"]) else np.nan), axis=1)
    df["economic_eff_raw"] = df["economic_eff_raw"].replace([np.inf, -np.inf], np.nan).fillna(0.0)

    # Normalized dimensions
    df["C_norm"] = normalize_minmax(pd.to_numeric(df["Correctness"],         errors="coerce").fillna(0))
    # Economic efficiency normalization (higher is better)
    df["E_norm"] = normalize_minmax(df["economic_eff_raw"].astype(float).fillna(0.0))
    df["L_norm"] = normalize_minmax(pd.to_numeric(df["Execution Time (ms)"], errors="coerce").fillna(0), invert=True)
    ...
    if failure_count_override is not None:
        ...
        # Accept CSV values in either [0,1] or [0,100] scale.
